...
export interface CacheRepairReport {
  ...
  cleared: number
  /** legacy files left on disk because no row could prove a claim */
  orphaned: number
}
 ...
 * when it ALONE matches the file's size, its size is known and non-zero,
 * and the file was written under its own filename; it keeps the file,
 * renamed to the new scheme. Every other row forgets its path and
 * re-downloads on next open. Unclaimed files stay on disk: cache bytes may
 * be the last copy of media the CDN has since expired, so nothing here is
 * destructive. Idempotent — after one pass no legacy paths remain. One
 * transaction, so a mid-way DB failure leaves nothing half-repaired.
 */
export function repairLegacyAttachmentCache(db: DbDriver): CacheRepairReport {
  const report: CacheRepairReport = { kept: 0, cleared: 0, orphaned: 0 }
    ...
    for (const [path, rows] of groups) {
      ...
      if (winner) {
        ...
        try {
          ...
          // rename refused (permissions, name too long): give up the claim
